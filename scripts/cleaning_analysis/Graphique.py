...
class graphique:
    ...
    def tab_pays_2(self, effectif):
        stats_localisation = self.df.groupby('pays')['prix'].agg(['mean', 'min', 'max', 'std']).reset_index()
    ...
    def tableau(self, colonne):
        stat = self.df.groupby(colonne)['prix'].agg(['mean', 'min', 'max', 'std']).reset_index()
        stat.columns = [colonne, 'prix_moyen', 'prix_min', 'prix_max', 'ecart_type']
        stat['prix_moyen'] = stat['prix_moyen'].round(2)
        stat['ecart_type'] = stat['ecart_type'].round(2)
        stat = stat.sort_values(by="prix_moyen", ascending=False).reset_index(drop=True)


        fig = go.Figure(data=[go.Table(
            header=dict(values=[colonne, "prix_moyen","prix_min", "prix_max", "ecart_type"],
                        fill_color='paleturquoise',
                        align='center'),
            cells=dict(values=[stat[colonne], stat['prix_moyen'], stat['prix_min'], stat['prix_max'], stat['ecart_type']],
                    fill_color='lavender',
                    align='left'))
        ])
    ...
    def tableau_gt(self, colonne):
        ...
        table = (
            gt.GtTable.from_pandas(stat)
            .set_title(f"Statistiques par {colonne}")
            .set_column_labels({
                colonne: colonne,
            ...
            .format(precision=2)  # Arrondir toutes les colonnes numériques à 2 décimales
            .set_row_striping(bg_color='lavender')
            .set_header_color(bg_color='paleturquoise')
        )
    ...
    def boxplot_plotly(self, colonne):
        ...
        fig = px.box(
            ...
            labels={'prix_log': 'Prix des montres (log)', colonne: f'Types de {colonne}'},
        ...
        fig.update_layout(
            ...
            xaxis_title='Prix des montres (log)',
            showlegend=False
        )
